...
export const useEventTracker = () => {
    ...
    useEffect(() => {
        ...
        worker.current = new Worker(new URL('../lib/analytics-worker.ts', import.meta.url));
        ...
        worker.current?.postMessage({
            type: 'INIT',
            data: { sessionId, currentPath: page, pageLoadTime: Date.now(), viewportWidth: window.innerWidth, viewportHeight: window.innerHeight, baseUrl },
        });
        ...
        const syncToBackend = async () => {
            ...
            const flattenedEvents = allEvents.map(event => {
                ...
                if (event.type === 'click') {
                    ...
                    return { ...baseEvent, duration: Date.now() - event.data.pageLoadTime }; // Update duration dynamically
                }
                return baseEvent;
            });
            ...
            try {
                ...
                if (response.ok) {
                    console.log('✅ Synced to backend:', flattenedEvents.length, 'events');
        ...
        const handleActivity = () => {
            worker.current?.postMessage({ type: 'ACTIVITY', data: { activityType: 'user_interaction' } });
